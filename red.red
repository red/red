...
redc: context [
	...
	load-targets: func [/local targets] [
		targets: load %system/config.red
		if exists? %system/custom-targets.red [
			insert targets load %system/custom-targets.red
		]
		targets
	]
	...
	main: func [/with cmd [string!] /local src opts build-dir prefix result file][
		...
		unless encap? [save %build/git.r do %build/git-version.red]
		
		print [lf "-=== Red Compiler" read %encapper/version.r "===-" lf]
		...
		if result: compile src opts [
			...
			if all [word: in opts 'packager get word][
				file: join %system/formats/ [opts/packager %.red]
