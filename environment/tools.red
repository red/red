		...
		data: context [
			...
			collector: function [
				...
				unless any [
					...
					change/only back tail topexs ccopy
					change/only back tail subexs ccopy
					if code [change/only back tail orgexs skip code offset]
				]
				...
				switch event [
					...
					fetch [								;-- save original values pushed to the stack
						...
						append/only stack either series? :value [copy/deep value][:value]
					]
					push [append/only evstack :value]	;-- save evaluated values pushed to the stack
					
					open [								;-- mark start of a sub-expression
						unless code [exit]				;@@ temp workaround for do/next
						stkpos: back tail stack			;-- back because func name is already on the stack
						...
						append/only stkexs stkpos
						
						append/only/dup orgexs skip code offset 2
						append/only/dup subexs skip last blocks offset 2
						incr back tail level
					]
					call [append path any [ref <anon>]]		;-- collect evaluation path
					return [							;-- revert both
						unless code [exit]				;@@ temp workaround for do/next
					...
					enter [								;-- mark start of an inner block of top-level exprs
						append/only stkexs tail stack
						append/only blocks c2: copy/deep code
						append/only orgblk code
						append/only level  0
						
						append/only/dup topexs c2 2
						append/only/dup orgexs code 2
						append/only/dup subexs c2 2
					]
					exit [								;-- revert it
						stkpos: take/last stkexs
						clear skip tail evstack negate length? stkpos 
						clear stkpos
						take/last blocks
						take/last orgblk
						take/last level
						
						clear back back tail topexs
						clear back back tail orgexs
						clear back back tail subexs
					]
					expr [								;-- remove unused expressions from the stack
						stkpos: last stkexs
						clear skip evstack negate length? stkpos 
						clear stkpos
						
						if 0 = last level [change/only back back tail topexs last topexs]
						change/only back back tail subexs last subexs
						change/only back back tail orgexs last orgexs
					]
				]
			...
			last-path: []								;-- cached, reported only when changed
		 	...
		 	inspect: function [
				...
				either data/isubex? [
					...
					p: tail data/topexs
					bgn: -2  end: -1
					expr: either 'error = event [p/:bgn][copy/part p/:bgn p/:end]
				]
				...
				unless any [data/isubex?  data/path == last-path] [
					...
					t: tail data/topexs
					bgn: -4  end: -3					;-- -4..-3 is the parent expression
					pexpr: any [if t/:bgn [copy/part t/:bgn t/:end] []]
