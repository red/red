...
virtual-struct!: context [
	...
	pad: func [buf [binary! string!] n [integer!] /local m][
		unless any [
			empty? buf
			zero? m: (length? buf) % n
		][	
			append/dup tail buf null n - m
		]
	]
	...
	set 'make-struct func [
		...
		if data [
			specs: skip keys-of obj 2					;-- skip over: __vs-type, __vs-spec
			until [
	...
	set 'form-struct func [
		...
		out: make binary! 4 * length? members: skip keys-of obj 2		;-- raw guess
		n: any [n alignment]
		...
		foreach name members [
			...
			append out switch/default type/1 [
				...
				float!   [pad out 4 #{0000000000000000}]	;-- placeholder
			][
