...
===start-group=== "unevaluated literals"
	--test-- "type?ul-1"
	--assert integer! = type? first [1]
	--test-- "type?ul-2"
	--assert char! = type? first [#"a"]
	--test-- "type?ul-3"
	--assert block! = type? first [[]]
	--test-- "type?ul-4"
	--assert get-word! = type? first [:b]
	--test-- "type?ul-5"
	--assert logic! = type? first [#[true]]
	--test-- "type?ul-6"
	--assert issue! = type? first [#1]
	--test-- "type?ul-7"
	--assert none! = type? first [#[none]]
	--test-- "type?ul-8"
	--assert paren! = type? first [(1)]
	--test-- "type?ul-9"
