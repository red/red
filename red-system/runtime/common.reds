...
***-on-quit: func [							;-- global exit handler
	...
	unless zero? status [
		...
		;if status =  1 [msg: ""]			;-- reserved
		if status =  2 [msg: "access violation"]
		if status =  3 [msg: "invalid alignment"]
		if status =  4 [msg: "breakpoint"]
		if status =  5 [msg: "single step"]
		if status =  6 [msg: "bounds exceeded"]
		if status =  7 [msg: "float denormal operan"]
		if status =  8 [msg: "float divide by zero"]
		if status =  9 [msg: "float inexact result"]
		if status = 10 [msg: "float invalid operation"]
		if status = 11 [msg: "float overflow"]
		if status = 12 [msg: "float stack check"]
		if status = 13 [msg: "float underflow"]
		if status = 14 [msg: "integer divide by zero"]
		if status = 15 [msg: "integer overflow"]
		if status = 16 [msg: "privileged instruction"]
		if status = 17 [msg: "invalid virtual address"]
		if status = 18 [msg: "illegal instruction"]
		if status = 19 [msg: "non-continuable exception"]
		if status = 20 [msg: "stack error or overflow"]
		if status = 21 [msg: "invalid disposition"]
		if status = 22 [msg: "guard page"]
		if status = 23 [msg: "invalid handle"]
		if status = 24 [msg: "illegal operand"]
		if status = 25 [msg: "illegal addressing mode"]
		if status = 26 [msg: "illegal trap"]
		if status = 27 [msg: "coprocessor error"]
		if status = 28 [msg: "non-existant physical address"]
		if status = 29 [msg: "object specific hardware error"]		
		if status = 30 [msg: "hardware memory error consumed AR"]
		if status = 31 [msg: "hardware memory error consumed AO"]
