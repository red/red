Red/System [
	Title:   "Red/System x86-64 struct! datatype test script"
	Author:  "Nenad Rakocevic"
	File: 	 %struct-x64-test.reds
	Tabs:	 4
...
struct-x64-int!: alias struct! [value [integer!]]
struct-x64-byte!: alias struct! [value [byte!]]
struct-x64-wide!: alias struct! [
	v1 [integer!]
	v2 [byte!]
	v3 [c-string!]
]

===start-group=== "Contiguous struct arithmetic"

	--test-- "struct-x64-int-1"
	int-size: size? struct-x64-int!
	int-buffer: allocate 3 * int-size
	int-a: as struct-x64-int! int-buffer
	int-b: as struct-x64-int! (int-buffer + int-size)
	int-c: as struct-x64-int! (int-buffer + (2 * int-size))
	int-a/value: 123
	int-b/value: 456
	int-c/value: 789
	--assert int-a/value = 123
	--assert int-b/value = 456
	--assert int-c/value = 789
	int-cursor: int-a
	--assert int-cursor/value = 123

	--test-- "struct-x64-int-2"
	int-cursor: int-a + 1
	--assert int-cursor/value = 456

	--test-- "struct-x64-int-3"
	int-cursor: int-c
	--assert int-cursor/value = 789
	int-cursor: int-c - 2
	--assert int-cursor/value = 123

	--test-- "struct-x64-int-4"
	struct-x64-index: 2
	int-cursor: int-a + struct-x64-index
	--assert int-cursor/value = 789

	--test-- "struct-x64-int-5"
	struct-x64-index: -2
	int-cursor: int-c + struct-x64-index
	--assert int-cursor/value = 123

	--test-- "struct-x64-int-6"
	int-cursor: int-a - struct-x64-index
	--assert int-cursor/value = 789
	free int-buffer

	--test-- "struct-x64-byte-1"
	byte-size: size? struct-x64-byte!
	byte-buffer: allocate 3 * byte-size
	byte-a: as struct-x64-byte! byte-buffer
	byte-b: as struct-x64-byte! (byte-buffer + byte-size)
	byte-c: as struct-x64-byte! (byte-buffer + (2 * byte-size))
	byte-a/value: #"a"
	byte-b/value: #"b"
	byte-c/value: #"c"
	--assert byte-a/value = #"a"
	--assert byte-b/value = #"b"
	--assert byte-c/value = #"c"
	byte-cursor: byte-a
	--assert byte-cursor/value = #"a"

	--test-- "struct-x64-byte-2"
	byte-cursor: byte-a + 1
	--assert byte-cursor/value = #"b"

	--test-- "struct-x64-byte-3"
	byte-cursor: byte-c
	--assert byte-cursor/value = #"c"
	byte-cursor: byte-c - 2
	--assert byte-cursor/value = #"a"

	--test-- "struct-x64-byte-4"
	struct-x64-index: 2
	byte-cursor: byte-a + struct-x64-index
	--assert byte-cursor/value = #"c"

	--test-- "struct-x64-byte-5"
	struct-x64-index: -2
	byte-cursor: byte-c + struct-x64-index
	--assert byte-cursor/value = #"a"

	--test-- "struct-x64-byte-6"
	byte-cursor: byte-a - struct-x64-index
	--assert byte-cursor/value = #"c"
	free byte-buffer

	--test-- "struct-x64-wide-1"
	wide-size: size? struct-x64-wide!
	wide-buffer: allocate 3 * wide-size
	wide-a: as struct-x64-wide! wide-buffer
	wide-b: as struct-x64-wide! (wide-buffer + wide-size)
	wide-c: as struct-x64-wide! (wide-buffer + (2 * wide-size))
	wide-a/v1: 123
	wide-a/v2: #"a"
	wide-a/v3: "A"
	wide-b/v1: 456
	...
	wide-c/v2: #"c"
	wide-c/v3: "C"
	--assert wide-a/v1 = 123
	--assert wide-b/v1 = 456
	--assert wide-c/v1 = 789
	--assert wide-a/v2 = #"a"
	--assert wide-b/v2 = #"b"
	--assert wide-c/v2 = #"c"
	--assert wide-a/v3/1 = #"A"
	--assert wide-b/v3/1 = #"B"
	--assert wide-c/v3/1 = #"C"
	wide-cursor: wide-a
	--assert wide-cursor/v1 = 123
	--assert wide-cursor/v2 = #"a"
	--assert wide-cursor/v3/1 = #"A"

	--test-- "struct-x64-wide-2"
	wide-cursor: wide-a + 1
	--assert wide-cursor/v1 = 456
	--assert wide-cursor/v2 = #"b"
	--assert wide-cursor/v3/1 = #"B"
	free wide-buffer

===end-group===

===start-group=== "Local contiguous struct arithmetic"

struct-x64-local: func [
	/local
		int-size byte-size wide-size struct-x64-index [integer!]
		int-buffer byte-buffer wide-buffer [byte-ptr!]
		int-a int-b int-c int-cursor [struct-x64-int!]
		byte-a byte-b byte-c byte-cursor [struct-x64-byte!]
		wide-a wide-b wide-c wide-cursor [struct-x64-wide!]
][
	--test-- "loc-struct-x64-int-1"
	int-size: size? struct-x64-int!
	int-buffer: allocate 3 * int-size
	int-a: as struct-x64-int! int-buffer
	int-b: as struct-x64-int! (int-buffer + int-size)
	...
	--test-- "loc-struct-x64-int-2"
	int-cursor: int-a + 1
	--assert int-cursor/value = 456

	--test-- "loc-struct-x64-int-3"
	int-cursor: int-c
	--assert int-cursor/value = 789
	int-cursor: int-c - 2
	--assert int-cursor/value = 123

	--test-- "loc-struct-x64-int-4"
	struct-x64-index: 2
	int-cursor: int-a + struct-x64-index
	--assert int-cursor/value = 789

	--test-- "loc-struct-x64-int-5"
	struct-x64-index: -2
	int-cursor: int-c + struct-x64-index
	--assert int-cursor/value = 123

	--test-- "loc-struct-x64-int-6"
	int-cursor: int-a - struct-x64-index
	--assert int-cursor/value = 789
	free int-buffer

	--test-- "loc-struct-x64-byte-1"
	byte-size: size? struct-x64-byte!
	byte-buffer: allocate 3 * byte-size
	byte-a: as struct-x64-byte! byte-buffer
	byte-b: as struct-x64-byte! (byte-buffer + byte-size)
	...
	--test-- "loc-struct-x64-byte-2"
	byte-cursor: byte-a + 1
	--assert byte-cursor/value = #"b"

	--test-- "loc-struct-x64-byte-3"
	byte-cursor: byte-c
	--assert byte-cursor/value = #"c"
	byte-cursor: byte-c - 2
	--assert byte-cursor/value = #"a"

	--test-- "loc-struct-x64-byte-4"
	struct-x64-index: 2
	byte-cursor: byte-a + struct-x64-index
	--assert byte-cursor/value = #"c"

	--test-- "loc-struct-x64-byte-5"
	struct-x64-index: -2
	byte-cursor: byte-c + struct-x64-index
	--assert byte-cursor/value = #"a"

	--test-- "loc-struct-x64-byte-6"
	byte-cursor: byte-a - struct-x64-index
	--assert byte-cursor/value = #"c"
	free byte-buffer

	--test-- "loc-struct-x64-wide-1"
	wide-size: size? struct-x64-wide!
	wide-buffer: allocate 3 * wide-size
	wide-a: as struct-x64-wide! wide-buffer
	wide-b: as struct-x64-wide! (wide-buffer + wide-size)
