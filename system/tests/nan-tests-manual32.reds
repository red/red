Red/System [
	Title:   "Red/System manual test of IEEE-754 compliance of NaN comparison - float32 version"
	Author:  "@hiiamboris"
	File: 	 %float-matrix-manual32.reds
	Tabs:	 4
...
;; check point 7 - all's
       all [zero = zero  zero < one  one < two  one <> nan  nan <> nan  nan = nan  print-line "FAILED: all ... nan = nan 1"]
if     all [zero = zero  zero < one  one < two  one <> nan  nan <> nan  nan = nan][print-line "FAILED: all ... nan = nan 2"]
unless all [zero = zero  zero < one  one < two  one <> nan  nan <> nan][print-line "FAILED: unless all ..."]
...
;; ... any's
       any [zero <> zero  zero > one  one > two  one = nan  nan = nan  nan <> nan  print-line "FAILED: any ... nan <> nan 1"]
unless any [zero <> zero  zero > one  one > two  one = nan  nan = nan  nan <> nan][print-line "FAILED: any ... nan <> nan 2"]
either any [zero <> zero  zero > one  one > two  one = nan  nan = nan] [print-line "FAILED: either any ..."] [print-line "OK: either any ..."]
...
if all [zero / zero <= +inf]      [print-line "FAILED: if all [0.0 / 0.0 <= +inf] 1"]
if all [true zero / zero <= +inf] [print-line "FAILED: if all [0.0 / 0.0 <= +inf] 2"]
if all [zero / zero <= +inf true] [print-line "FAILED: if all [0.0 / 0.0 <= +inf] 3"]

if     any [zero / zero <= +inf]       [print-line "FAILED: if any [0.0 / 0.0 <= +inf] 1"]
if     any [zero / zero <= +inf 1 = 2] [print-line "FAILED: if any [0.0 / 0.0 <= +inf] 2"]
unless any [true zero / zero <= +inf]  [print-line "FAILED: if any [0.0 / 0.0 <= +inf] 3"]
unless any [zero / zero <= +inf true]  [print-line "FAILED: if any [0.0 / 0.0 <= +inf] 4"]
