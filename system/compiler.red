...
#include %utils/profiler.red
profiler/active?: no

#include %utils/int-to-bin.red
#include %utils/IEEE-754.red
...
system-dialect: make-profilable context [
	...
	loader: do bind load %loader.red self
	...
	compiler: make-profilable context [
      ...
      (to-word "%")	[2	op		- [a [number!] b [number!] return: [number!]]]		;-- remainder (real syntax: %)
		...
		variadic?: func [value][all [issue? value #"." <> first to-string value]]
		...
		preprocess-array: func [list [block!] /local p s v][
			parse list [
				some [
					...
					| string! | char! | integer! | float! | get-word!
		...
		ns-decorate: func [path [path!] /global /set /local name][
			name: replace/all mold path slash decoration
			to get pick [set-word! word!] to logic! set name
		]
		...
		check-arguments-type: func [name args /local entry spec list type][
			...
			forall args [
				append/only list either all [
					find [float! issue!] type?/word args/1
					spec/2/1 = 'float32!
				][
		...
		expand-setwords: has [list p lit? value out][
			...
			lit?: find [integer! float! char! logic!] type?/word p/2
			...
			parse body rule: [
				while [
					'loop pos: (
						unless issue? pos/-2 [
							loops: loops + 1
							insert pos append copy @L.L loops
							pos: next pos
						]
					)
					| p: block! :p into rule | skip
				]
			]	
			unless pos: find spec /local [pos: append spec /local]
			repeat i loops [append pos reduce [to-word form append copy "L.L" i [integer!]]] ;-- add counters to locals
		]
		
		comp-loop: has [name expr body start counter vars pos][		
			pc: next pc
			if ref? pc/1 [name: to-word form pc/1 remove pc] ;-- process loop's hidden counter variable
		...
		comp-expression: func [
			...
			either block? expr [
				...
				last-type: either not any [
					...
					either all [boxed not float? unbox expr not float? boxed/data][
		...
		comp-dialect: has [expr][
			...
			while [not tail? pc][
				case [
					all [issue? pc/1 #"." <> first to-string pc/1][comp-directive]
					all [
						set-word? pc/1
	...
	make-job: func [opts [object!] file [file!] /local job blk][
		blk: body-of opts
		forall blk [
			if lit-word? blk/1 [blk/1: to-word blk/1]
		]
		job: construct/with blk linker/job-class	
