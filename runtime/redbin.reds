...
redbin: context [
	...
	preprocess-binding: func [
		...
		either type = TYPE_OBJECT [
			...
			object/class:  either codec? [-1][data/2]	;@@ TBD: concurrent class IDs — codec forces a fresh id
	...
	referral-node?: func [							;-- TRUE only if the target holds a series node in data2 — the slot
		value   [red-value!]						;-- every series decoder re-heads as its node. WHITELIST, not blacklist:
		return: [logic!]							;-- a reference only ever targets such a value; anything else (object=class,
		/local t [integer!]							;-- word=symbol, point=coord, scalar=data in data2) is a crafted/corrupt payload.
	][
		t: TYPE_OF(value)
		any [
			ANY_SERIES?(t)								;-- block/paren/paths/hash/strings/binary/vector/image: data2 = series node
			t = TYPE_MAP								;-- data2 = block node
			t = TYPE_BITSET								;-- data2 = bitset series node
			ALL_FUNCTION?(t)							;-- function/action/native/routine/op: data2 = spec block node
		]
	]
	...
	encode-object-cp: func [
		...
		object: as red-object! data
		owner?: all [not ref? not zero? object/on-set]
	...
	encode-word-cp: func [
		...
		handle: node-handle-from-cell data/data1

		;@@ TBD: #4537
		if zero? handle [handle: global-ctx]
		node: resolve-node handle
		...
		either all [handle = global-ctx not ref?][	;-- canonical form: global binding, symbol only
			emit-tag payload type no
			emit-varint payload encode-symbol-cp data table symbols strings
		][
			emit-tag payload type yes
	...
	decode-binding-cp: func [							;-- consume an object!/function! carrier record,
		...
		context: as red-context! alloc-tail series
		context/header: TYPE_UNSET
		context/symbols: node-handle-of _hashtable/init slots null HASH_TABLE_SYMBOL HASH_SYMBOL_CONTEXT
		context/values: node-handle-of values
		...
		either type = TYPE_OBJECT [
			...
			if owner? [
				vseries: resolve-series object/on-set
			...
			vseries: resolve-series fun/more
		...
		if bind-word <> null [bind-word/ctx: node-handle-of node]	;-- bind before values decode, so self-references resolve
		...
		if filled? [
			...
			vseries: resolve-series context/values
	...
	decode-context-cp: func [							;-- root-level context! record (boot payload)
		...
		new: _context/create slots stack? self? 0 kind
	...
	decode-block-cp: func [
		...
		either ref? [
			p: read-reference-cp p end value: cp-ref
			unless referral-node? value [throw-error-cp p]	;-- referral target must hold a series node (data2)
	...
	decode-string-cp: func [
		...
		either ref? [
			...
			str/cache:	0								;-- strings carry a node-handle cache; clear the fresh cell's
	...
	decode-bitset-cp: func [
		...
		either ref? [
			...
			if nl? [slot/header: slot/header or flag-new-line]
			set-type as red-value! slot TYPE_BITSET
			if mod? [set-flag resolve-node slot/node flag-bitset-not]
			p: p + size
		]
		p
	]
