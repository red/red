...
tls: context [
	...
	validate?: func [
		...
		unless CertGetCertificateChain null ctx null store para flags null :cert-chain [
			...
			print-line as int-ptr! platform/GetLastError
			CertCloseStore store 0
			return false
		]
					...
					loop 3 [
						...
						if buf/BufferType = 5 [	;-- some leftover, save it
							...
							if data/extra-sz < src-len [
								if data/tls-extra <> null [free data/tls-extra]
								data/tls-extra: allocate src-len
	...
	free-handle: func [
		...
		if td/tls-buf <> null [free td/tls-buf]
		if td/tls-extra <> null [free td/tls-extra]
	]
]