...
transform-grad: func [
	...
	if all [
		...
		type <> DRAW_BRUSH_GRADIENT_SMART
	][exit]
	if grad-type = linear [
		set-linear-transform brush
		exit
	]
	if grad-type = radial [
		set-radial-transform brush
		exit
	]
]
	...
	either pen-fill = -1 [
		...
		either pen-fill = pen [
			if all [
				...
				ctx/pen-type <> DRAW_BRUSH_GRADIENT_SMART
			][exit]
			bthis: ctx/pen
		][
			if all [
				ctx/brush-type <> DRAW_BRUSH_GRADIENT
				ctx/brush-type <> DRAW_BRUSH_GRADIENT_SMART
			][exit]
			bthis: ctx/brush
		]
